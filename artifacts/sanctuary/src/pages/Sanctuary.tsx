  ...
  return (
    <div className="flex h-[100dvh] w-full bg-background text-foreground overflow-hidden">
      ...
      <div className="md:hidden fixed top-0 left-0 right-0 h-14 border-b border-border bg-background/95 backdrop-blur-sm z-30 flex items-center px-4">
        ...
        <span className="font-serif ml-2 text-lg">Sanctuary</span>
      </div>
      ...
      <main className="flex-1 flex flex-col relative pt-14 md:pt-0 w-full overflow-hidden bg-[radial-gradient(ellipse_at_top_right,_var(--tw-gradient-stops))] from-sidebar/5 via-background to-background">
        <div className="absolute inset-0 z-0 pointer-events-none opacity-[0.02]" style={{ backgroundImage: "url('https://grainy-gradients.vercel.app/noise.svg')" }}></div>
        ...
        <div className="flex-1 relative z-10 w-full h-full">
          ...
          {view === "postbox" && activeTag && (
            <PostboxView tag={activeTag} onReadLetter={handleReadLetter} onNewLetter={handleOpenComposer} />
          )}

          {view === "chat" && activeTag && (
            <ChatView tag={activeTag} />
          )}

          {view === "composer" && (
            <LetterComposer onClose={handleCloseOverlay} initialTagId={activeTag?.type === "postbox" ? activeTag.id : undefined} />
          )}

          {view === "reader" && activeLetterId && (
            <LetterReader letterId={activeLetterId} onClose={handleCloseOverlay} />
          )}
        </div>
      </main>
    </div>
